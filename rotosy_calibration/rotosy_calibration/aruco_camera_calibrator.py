...
class ArucoCameraCalibrator(Node):
    def __init__(self):
        ...
        # cv_bridge 없이 ROS Image → numpy 변환
        self._encoding_channels = {
            'bgr8': (3, False), 'rgb8': (3, True),
            'bgra8': (4, False), 'rgba8': (4, True),
            'mono8': (1, False),
        }
    ...
    def _imgmsg_to_bgr(self, msg: Image) -> np.ndarray:
        """ROS2 Image 메시지 → BGR numpy 배열 (cv_bridge 없이)."""
        enc = msg.encoding.lower()
        if enc not in self._encoding_channels:
            raise ValueError(f'Unsupported encoding: {msg.encoding}')
        channels, need_flip = self._encoding_channels[enc]
        dtype = np.uint8
        if channels == 1:
