...
class MedicineBoxCenterDetector(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("yolo_medicine_conf", 0.5)
        self.declare_parameter("yolo_water_pack_conf", 0.5)
        self.declare_parameter("yolo_history_frames", 6)
        self.declare_parameter("yolo_track_iou", 0.5)
        self.declare_parameter("yolo_min_votes", 2)
        self.declare_parameter("yolo_switch_margin", 1.2)
        self.declare_parameter("yolo_inference_hz", 30.0)
        ...
        self.vision_detector = self._create_configured_vision_detector()
    ...
    def _create_configured_vision_detector(self) -> Optional[Any]:
        if not bool(self.get_parameter("use_yolo").value):
            return None
        if VisionDetector is None:
            self.get_logger().warn("VisionDetector unavailable; using legacy YOLO path.")
            return None

        device = str(self.get_parameter("yolo_device").value)
        if device.lower() in {"", "none"}:
            device = None

        detector = VisionDetector(
            medicine_conf=float(self.get_parameter("yolo_medicine_conf").value),
            water_pack_conf=float(self.get_parameter("yolo_water_pack_conf").value),
            history_frames=int(self.get_parameter("yolo_history_frames").value),
            track_iou=float(self.get_parameter("yolo_track_iou").value),
            min_votes=int(self.get_parameter("yolo_min_votes").value),
            switch_margin=float(self.get_parameter("yolo_switch_margin").value),
            device=device,
            inference_hz=float(self.get_parameter("yolo_inference_hz").value),
        )
        if not detector.load():
            self.get_logger().warn(
                f"VisionDetector disabled: {detector.error}; using legacy YOLO path."
            )
            return None
        return detector
    ...
    def process_frame(self) -> None:
        ...
        yolo_detections = self._detect_yolo_boxes(color, depth_m, depth_raw)
    ...
    def _detect_yolo_boxes(
        ...
        if self.vision_detector is not None and depth_raw is not None:
            stable, _ = self.vision_detector.process(
                color,
                depth_raw,
                self.depth_scale,
                self.intrinsics,
            ...
            detections = [
                self._yolo_detection_from_vision(item)
                for item in stable
            ]
    ...
    def _yolo_detection_from_vision(self, item: dict) -> Optional[YoloDetection]:
        bbox = item.get("bbox")
        ...
        point = item.get("camera_position_m")
        point_m = None
