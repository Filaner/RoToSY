...
파라미터:
  ...
  direction=3 (하강):
    사용자가 실제 로봇을 조그해 찍은 예시를 일반화한 동작.
      ...
      → Δy≈+223, Δz≈-127 (이 값을 그대로 쓰려면 radius=127, y_distance=223)
...
사용 예:
  ...
  ros2 run doosan_controller test_moveC 127 3 223    # 사용자 예시와 동일한 하강 호
...
class TestMoveCNode(Node):
    ...
    def __init__(self, radius_mm: float, direction: int, y_distance_mm: float | None = None):
        ...
        self._y_distance  = radius_mm if y_distance_mm is None else y_distance_mm
