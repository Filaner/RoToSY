...
Connects upstream to:
  ...
  Services: <ns>/motion/move_joint, <ns>/motion/move_line,
            ...
            <ns>/system/get_robot_state, <ns>/system/get_current_pose
"""
...
from robot_arm_interfaces.srv import ServoOn, WaitDuration
...
CONTROL_SERVO_ON         = 3   # alias (= CONTROL_RESET_SAFET_OFF in DRFC.h)
...
class ArmControllerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._robot_state: int   = STATE_NOT_READY
    ...
    def _update_pose_cache(self) -> None:
        """Refresh _current_joints and _current_tcp from the DSR driver."""
        req = GetCurrentPose.Request()
        req.space_type = 0   # 0 = joint space → fills joint positions
        resp_j = self._call_service_sync(self._cli_get_pose, req, timeout=2.0)
        if resp_j:
            self._current_joints = list(resp_j.pos)

        req.space_type = 1   # 1 = task (Cartesian) space
        resp_t = self._call_service_sync(self._cli_get_pose, req, timeout=2.0)
        if resp_t:
            self._current_tcp = list(resp_t.pos)
    ...
    def _do_servo_on(self):
        ...
        while time.time() < deadline:
            ...
            if s == STATE_STANDBY:
                ...
                log.info('Servo ON 성공 → STANDBY (빨간불 확인)')
    ...
    def _goal_callback(self, goal_request):
        state = self._get_robot_state_now()
        # SAFE_OFF는 execute 단계에서 자동 서보 ON 시도 → 수락
        if state in (STATE_STANDBY, STATE_MOVING, STATE_SAFE_OFF):
            return GoalResponse.ACCEPT
        self.get_logger().warn(
            f'Rejecting goal: state={STATE_STR.get(state, state)} — '
            'SAFE_STOP/NOT_READY는 티치펜던트에서 리셋 필요'
        )
    ...
    def _execute_movej(self, goal_handle) -> MoveJ.Result:
        ...
        if self._get_robot_state_now() == STATE_SAFE_OFF:
            log.info('SAFE_OFF 감지 → 자동 Servo ON 시도')
            ok, msg = self._do_servo_on()
            if not ok:
                result.success = False
                result.message = f'자동 Servo ON 실패: {msg}'
                goal_handle.abort()
                return result
        ...
        try:
            ...
            def feedback_thread():
                while not done_event.is_set():
                    ...
                    fb.robot_state = STATE_STR.get(self._get_robot_state_now(), 'UNKNOWN')
                    goal_handle.publish_feedback(fb)
                    time.sleep(0.5)

            fb_t = threading.Thread(target=feedback_thread)
            fb_t.start()
